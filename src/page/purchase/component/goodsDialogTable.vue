<template>
    ...
    <el-dialog
        ...
        <div  class="dialog-footer">
            ...
            <el-button type="primary" size="small" :disabled="!bindGoodsArr.length" @click="dialogSure">确 定（{{bindGoodsArr.length}}）</el-button>
        </div>
    </el-dialog>
</template>
  ...
  methods: {
    ...
    goodsSelTable (val) {
      if (this.disableGoodsId.indexOf(val.goods_id) > -1) {
        return false
      } else if (this.isReturn && !parseInt(val.stock)) {
        return false
      } else {
        return true
      }
