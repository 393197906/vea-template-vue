        ...
        <detailsFrame>
            <el-form ref="goodsForm" slot="basicInfo" :rules="goodsRules" :model="subData" label-width="130px" size="small" inline>
                <div>
                    ...
                    <el-form-item label="运费/装卸费：">
                        <el-input-number
                                v-model="subData.freightCharge"
            ...
            <div slot="selectGoods">
                ...
                <div class="table-info">
                    <p>共 {{selectGoodsData.length}} 件商品，合计金额 {{priceAllNum.toFixed(4)}} 元。</p>
                </div>
                <!-- 表格 -->
                <el-table style="width: 100%" :data="selectGoodsData">
                    ...
                    <el-table-column prop="date" align="center" label="小计(元)" >
                        <template slot-scope="scope">
                            <p>{{(scope.row.goodsWeight * (scope.row.price?scope.row.price:0)).toFixed(4)||0}}</p>
                        </template>
                    </el-table-column>
    ...
    saveWarehousing () {
      ...
      this.$refs.goodsForm.validate(async (valid) => {
        if (valid) {
          ...
          const {err} = await this.$service.purchase.addPurchaseSheet(params)
          this.submitLoading = false
          if (err) return
          this.$confirm('是否继续创建采购入库单？', '提示', {
