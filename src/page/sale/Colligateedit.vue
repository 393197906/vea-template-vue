      ...
      <div>
        <el-form ref="form" label-width="100px">
          ...
          <el-form-item label="优惠条件">
            <el-row>
              <el-col :span="8">
                满 <el-input-number v-model="formData.nums" class="price_input" :min="1"  :controls="false"></el-input-number>
                   件商品
              </el-col>
            </el-row>
          </el-form-item>
  ...
  methods: {
    ...
    async submitData () {
      ...
      const {
        err,
        data
      } = await this.$service.shopmanager.colligate.colligateEdit(
        this.id,
        this.formData
      )
      this.$message({
    ...
    /** 活动详情 */
    async _reductionDetail () {
      this.dialogTableVisible = true
      const {
        err,
        data
      } = await this.$service.shopmanager.colligate.colligateDetail(this.id)
      const _data = data.result
      this.formData.compre_name = _data.compre_name
      this.formData.start_time = _data.start_time
      this.formData.end_time = _data.end_time
      this.formData.range_type = _data.range_type + ''
      this.formData.nums = _data.rule[0].nums
      this.formData.remark = _data.remark
