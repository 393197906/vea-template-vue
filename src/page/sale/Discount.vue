<template>
  <div v-loading="loading">
    ...
    <div class="search-box">
      ...
      <router-link to="/sale/discount/discountsave" v-if="$checkActionAccess($ACCESS.SALE_XSZK_ADD)">
        <el-button size="small" type="primary" icon="el-icon-plus">添加限时折扣</el-button>
      </router-link>
      <div class="searcher">
    ...
    <el-row class="table-main" :key="dataItem.discount_id" v-for="(dataItem, i) in reduction_list">
      ...
      <el-col :span="4">
        <div class="table-content">
          <template v-if="$checkActionAccess($ACCESS.SALE_XSZK_EDIT)">
            ...
            <el-button type="text" v-if="dataItem.status===1&&$checkActionAccess($ACCESS.SALE_XSZK_CLOSE)"  @click="closeActive(dataItem.discount_id)">关闭</el-button>
            <el-button type="text" v-if="dataItem.status!==1&&$checkActionAccess($ACCESS.SALE_XSZK_DELETE)" @click="_reducitonDelete(dataItem.discount_id)">删除
            </el-button>
          </template>

          <el-button v-if="$checkActionAccess($ACCESS.SALE_XSZK_DETAIL)" type="text"
    ...
    <!--分页end-->
    <!-- 详情弹窗 -->
    <el-dialog title="限时折扣详情" :visible.sync="dialogTableVisible" :modal="false" class="data_detail_alert">
      <ul class="data_view">
          ...
          <div>
            ...
            <template v-else v-for="goodItem in data_detail.goods">
              <div class="goods_view clearfix">
                ...
                <span class="data_discount">{{goodItem.discount}}<span v-if="goodItem.discount">折</span> </span>
              </div>
            </template>
          </div>
...
export default {
  ...
  methods: {
    ...
    /** 编辑 */
    data_edit (id) {
      this.$router.push({name: 'discountedit', params: {id: id}})
    },
    /** 活动关闭 */
    closeActive (id) {
      this.$confirm('确定要关闭该活动?', '提示', {
        ...
        .then(async () => {
          const {
            err,
            data
          } = await this.$service.shopmanager.discount.discountStatus(id, {
            status: 3
          })
          this.$message({
            type: 'success',
    ...
    async _reductionDetail (id) {
      ...
      const {
        err,
        data
      } = await this.$service.shopmanager.discount.discountDetail(id)
      ...
      this.$confirm('确定要删除该活动?', '提示', {
        ...
        .then(async () => {
          const {
            err,
            data
          } = await this.$service.shopmanager.discount.discountDel(id)
          this.$message({
            type: 'success',
            message: data.message
          })
    ...
    async _reducitonList () {
      this.loading = true
      const {err, data} = await this.$service.shopmanager.discount.list(this.config)
      this.loading = false
      if (err) return
