...
<template>
  <div>
    ...
    <el-tabs class="active_box">
      <p class="view_title">选择活动商品<i style="margin-left: 10px;color: #6f7180; font-size: small">(限购数量为0时即不限制)</i></p>
      <div>
  ...
  methods: {
    ...
    async submitData () {
      ...
      const {
        err,
        data
      } = await this.$service.shopmanager.seckill.discountAdd(this.formData)
      this.loading = false
      if (err) return
      this.$message({
