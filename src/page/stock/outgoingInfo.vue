    ...
    <div>
      <el-form ref="form" :model="form" :rules="formRules" label-width="90px" class="demo-ruleForm" inline>
        ...
        <div v-if="!isHad" style="text-align:right;padding-bottom:20px;width:80%">
          <el-button @click="submitForm('form')" size="small" type="primary" :loading="loading"   v-if="$checkActionAccess($ACCESS.SELL_OUT)">提交出库</el-button>
        </div>
      </el-form>
    </div>
...
export default {
    ...
    submitForm (formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          const {err} = await this.$service.outgoing.editSellStatus({
            id,
            ...this.form
          })
          this.loading = false
  ...
  async created () {
    if (this.$route.params.id) {
      ...
      this.form.warehouseId = this.service.warehouseList[0].id
    } else {
      this.getData(this.id, this.isReturn)
    }
  }
}
</script>
