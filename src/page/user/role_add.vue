<template>
  <div class="form_view">
    <el-form ref="formValidate" :model="formValidate" :rules="ruleValidate" label-width="100px">
      ...
      <el-form-item label="权限分配" prop="id" id="role-add">
    ...
    async submitForm (name) {
      this.$refs[name].validate(async (valid) => {
        if (valid) {
          ...
          this.loading = true
          const {err, data} = await this.$service.member.roleAdd({
            ...this[name],
...
<style>
  #role-add .el-tree-node__content[style*="18"] + .el-tree-node__children .el-tree-node {
